...
int test_pos_fixnum_read() {
    ...
    mu_assert(minipack_unpack_pos_fixnum(data+0) == 0);
    mu_assert_msgpack_uint8(0, 1, data+0);
    mu_assert(minipack_unpack_pos_fixnum(data+1) == 2);
    mu_assert_msgpack_uint8(2, 1, data+1);
    mu_assert(minipack_unpack_pos_fixnum(data+2) == 20);
    mu_assert_msgpack_uint8(20, 1, data+2);
    mu_assert(minipack_unpack_pos_fixnum(data+3) == 127);
    mu_assert_msgpack_uint8(127, 1, data+3);
    mu_assert(minipack_unpack_pos_fixnum(data+4) == 0);
    return 0;
}

int test_pos_fixnum_write() {
    uint8_t data[] = {0x00, 0x00, 0x00, 0x00, 0x00};
    minipack_pack_pos_fixnum(data+0, 0);
    minipack_pack_pos_fixnum(data+1, 2);
    minipack_pack_pos_fixnum(data+2, 20);
    minipack_pack_pos_fixnum(data+3, 127);
    minipack_pack_pos_fixnum(data+4, 128);
...
int test_neg_fixnum_read() {
    ...
    mu_assert(minipack_unpack_neg_fixnum(data+0) == -1);
    mu_assert_msgpack_int8(-1, 1, data+0);
    mu_assert(minipack_unpack_neg_fixnum(data+1) == -3);
    mu_assert_msgpack_int8(-3, 1, data+1);
    mu_assert(minipack_unpack_neg_fixnum(data+2) == -20);
    mu_assert_msgpack_int8(-20, 1, data+2);
    mu_assert(minipack_unpack_neg_fixnum(data+3) == -32);
...
int test_neg_fixnum_write() {
    ...
    minipack_pack_neg_fixnum(data+0, -1);
    minipack_pack_neg_fixnum(data+1, -3);
    minipack_pack_neg_fixnum(data+2, -20);
    minipack_pack_neg_fixnum(data+3, -32);
